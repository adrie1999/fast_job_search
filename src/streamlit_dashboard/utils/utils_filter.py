...
def sidebar_filters(
    similarity_columns: list, min_max_values: dict, page_name: str, sort_by: bool
):
    """
    ...
    with st.sidebar:
        ...
        search_title = st.text_input(
            "Search Job Title", key=f"{page_name}_search_title"
        )
        search_company = st.text_input(
            "Search Company", key=f"{page_name}_search_company"
        )
        search_location = st.text_input(
            "Search Location", key=f"{page_name}_search_location"
        )
        ...
        if sort_by:
            sort_by = st.selectbox(
                "Sort by", similarity_columns, index=0, key=f"{page_name}_sort_by"
            )
        else:
            sort_by = None
...
def filter_data(
    ...
    if search_title:
        filtered_data = filtered_data[
            filtered_data["job_title"].str.contains(search_title, case=False, na=False)
        ]
    if search_company:
        filtered_data = filtered_data[
    ...
    if search_location:
        filtered_data = filtered_data[
            filtered_data["job_location"].str.contains(
                search_location, case=False, na=False
            )
        ]
