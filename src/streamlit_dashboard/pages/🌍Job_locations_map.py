...
filters, search_title, search_company, search_location, sort_by = sidebar_filters(similarity_columns, min_max_values, page_name='job_location',sort_by=False)
    ...
    if pd.notna(row['latitude']) and pd.notna(row['longitude']):
        try:
            ...
            if pd.notna(row["job_url"]):
                popup_html = f'<a href="{row["job_url"]}" target="_blank">{row["job_title"]}</a>'
            else:
