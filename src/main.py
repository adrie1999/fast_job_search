...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler(os.path.join(log_dir, 'scraping_process.log'))  # Log to a file in 'logs' folder
    ]
)
...
if __name__ == '__main__':
    ...
    path_config_scrapping = os.path.join(base_dir, 'src/web_scrapping', 'config_scrapping.yaml')
    ...
    web_scrapper.save_listing_to_parquet(job_listings_full,saving_path=path_save_scrapping_parquet)
    ...
    ranked_jobs = similarity_jobs_vs_cv(path_scrapped_parquet, cv_text_example, preferences_example, top_n=1000)
    ...
    ranked_jobs.to_parquet(os.path.join(path_data_save_streamlit,'data_streamlit.parquet'))
