...
class JobsMatcherCV:
    def __init__(self, path_data: str, model_name: str = 'sentence-transformers/distiluse-base-multilingual-cased-v1'):
        """
    ...
    def data(self) -> pd.DataFrame:
        ...
        try:
            ...
            df = df.drop_duplicates(subset=["job_title", "company_name", "job_description"])
            df["job_title"] = df["job_title"].str.replace(r"\swith verification", "", regex=True)
            return df
    ...
    def rank_jobs(self, cv_text: str, preferences: Dict[str, str], top_n: int = 10) -> pd.DataFrame:
        """
        ...
        job_language_texts=[f"Language of the text : {get_language_name(job_text)}  Job offer: {job_text}" for job_text in job_texts]
        ...
        overall_similarity = cosine_similarity([cv_embedding.cpu().numpy()], job_embeddings.cpu().numpy()).flatten()
        ...
        for category, preference_text in preferences.items():
            if category == "title":
                ...
                category_similarity = cosine_similarity([preference_embedding.cpu().numpy()], job_title_embeddings.cpu().numpy()).flatten()
            elif category == "location":
                preference_embedding = self.get_embeddings([preference_text])[0]
                category_similarity = cosine_similarity([preference_embedding.cpu().numpy()], job_location_embeddings.cpu().numpy()).flatten()
            if category == "title":
                preference_embedding = self.get_embeddings([preference_text])[0]
                category_similarity = cosine_similarity([preference_embedding.cpu().numpy()], job_title_embeddings.cpu().numpy()).flatten()
            elif category == "language":
                preference_embedding = self.get_embeddings([preference_text])[0]
                category_similarity = cosine_similarity([preference_embedding.cpu().numpy()], job_language_embeddings.cpu().numpy()).flatten()
            else:
                preference_embedding = self.get_embeddings([preference_text])[0]
                category_similarity = cosine_similarity([preference_embedding.cpu().numpy()], job_embeddings.cpu().numpy()).flatten()
        ...
        ranked_jobs = self.data.sort_values(by="overall_similarity", ascending=False).head(top_n)
        
        return ranked_jobs[["job_title", "company_name", "job_location", "job_url",'job_description', "overall_similarity"] + [f"{cat}_similarity" for cat in category_scores.keys()]]

def similarity_jobs_vs_cv(path_data: str, cv_text: str, preferences: Dict[str, str], top_n: int = 10) -> pd.DataFrame:
    """
